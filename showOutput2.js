	...
	function() {
		...
		*************/
		...
		for ( i=0 ; i<keymapHeight; i++ ) {
			keymapData += " ".repeat( ( keymapWidth * 9 ) + 8 ) + "\\";
		}
		var keymapData_rowOffset = ( keymapWidth * 9 ) + 9;
		*/
		...
		for ( layer=0; layer<=15; layer++ ) {
			...
			for ( key=0; key<keys; key++ ) {
				...
				if ( key < keys-1 ) {
					//layerData2 += ",";
					//layerData2 += " ".repeat( Math.max(0, (8 - append.length) ) );
					
					/* DISABLE LINE WRAPPING BASED ON MATRIX DIMENSIONS
					if ( key % matrix_cols == matrix_cols-1 ) {
						layerData += "\\\n        ";
					} else {
						//layerData += "\\"
					}*/
				} else {
					//layerData2 += " ".repeat( Math.max(0, (9 - keycode.length) ) );
					//layerData2 += "\\" /*+ " ".repeat( Math.max(0, (9 - keycode.length) ) )*/;
				}
		...
		**************/
		...
		/****************************
		** CREATE KEYMAP CONTAINER **
		****************************/
		var keymapText = "";
		var keymap = new Array( layerCount );

		for ( layer=0 ; layer<layerCount; layer++ ) {
			keymap[layer] = Array(keymapHeight);
			for ( i=0 ; i<keymapHeight; i++ ) {
				keymap[layer][i] = Array(keymapWidth);
			}
		}
		...
		for ( key=0; key<keys; key++ ) {
			...
			for ( layer = 0; layer<layerCount; layer++ ) {
				...
				**********************/
				...
				keymap[layer][ Math.floor( y ) ][ Math.floor( x ) ] = keycode;
			}
		}
		/*
		...
		*/
		...
		for ( key=0; key<keys; key++ ) {
			...
			/*
			physicalMatrix += keyObjects[key][2].replace(/ \([0-9A-Z,]{5}\)/g, "") + ", ";
			if ( keyObjects[key][1] >= ( obj.keyboard.bounds.max.x - 1 ) ) {
				if ( key < ( keys - 1 ) ) {
		...
		*/
		...
		if ( i == layoutString.length-1 ) {
			layoutMacroOutput.splice(20, 0, ""+ physicalMatrix.replace(/, $/, "") +"  \\" );
		} else {
			layoutMacroOutput.splice(20, 0, ""+ physicalMatrix +"" );
		}
		//}
		*/
